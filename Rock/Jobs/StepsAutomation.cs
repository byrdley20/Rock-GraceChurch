...
namespace Rock.Jobs
{
    ...
    public class StepsAutomation : IJob
    {
            ...
            out List<string> errorMessages )
        {
            ...
            List<double> _stepElapsedMSList = new List<double>();
            foreach ( var personIdInfo in personInfoList )
            {
                var stopwatchStepElapsed = Stopwatch.StartNew();
                ...
                stopwatchStepElapsed.Stop();
                var stepElapsedMS = stopwatchStepElapsed.Elapsed.TotalMilliseconds;
                _stepElapsedMSList.Add( stepElapsedMS );

                progressCount++;
                if ( ( RockDateTime.Now - _lastProgressUpdate ).TotalSeconds >= 3 )
                {
                    try
                    {
