...
" );
        ...
        private void WriteSystemGuidTypeScriptFiles( string rootFolder )
        {
            ...
            foreach ( var type in types )
            {
                ...
                sb.AppendLine( AutoGeneratedLicense );
                sb.AppendLine();
                sb.AppendLine( $"declare const enum {type.Name} {{" );
                ...
                sb.AppendLine( "}" );
                sb.AppendLine();
                sb.AppendLine( $"export default {type.Name};" );

                file = new FileInfo( Path.Combine( rootFolder, "CodeGenerated", $"{type.Name}.d.ts" ) );
                WriteFile( file, sb );
            }
        ...
        private void WriteViewModelTypeScriptIndexFile( string rootFolder, IEnumerable<Type> types )
        {
            ...
            {
                sb.AppendLine( $"import {name} from './{name}';" );
            }
